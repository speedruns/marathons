module SeriesController
  ...
  def index(env)
    ...
    Template.render(env, "inventory/series/index.html.j2", {
      "series_list" => series
    })
  end
  ...
  def show(env)
    ...
    if series = Inventory.get_series(series_id)
      Template.render(env, "inventory/series/show.html.j2", {
        "series" => series.to_h
      })
    else
      env.redirect("/series")
  ...
  def _new(env)
    ...
    Template.render(env, "inventory/series/new.html.j2", {
      "series" => series.to_h
    })
  end
  ...
  def edit(env)
    series_id = env.params.url["series_id"]
    if series = Inventory.get_series(series_id)
      Template.render(env, "inventory/series/edit.html.j2", {
        "series" => series.to_h
      })
    else
      env.redirect("/series")
